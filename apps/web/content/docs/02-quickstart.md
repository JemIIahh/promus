...
14. If a subname was chosen, agent claims `<subname>.anima.0g` via the permissionless `PromusSubnameRegistrar` and writes the `address` plus `pubkey` text records.
