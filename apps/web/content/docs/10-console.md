...
3. **List iNFTs.** The page scans `ERC-721 Transfer(0x0 -> you)` logs on `PromusAgentNFT` and renders one row per token you currently own. Sync count and last-anchored timestamp come from `Updated` events.
