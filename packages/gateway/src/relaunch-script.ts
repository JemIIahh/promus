 ...
 * bootstrapped via git-clone (gateway at `$HOME/promus/packages/gateway/bin/`)
...
const PROGRESS_LOG = '/tmp/promus-relaunch-progress.log'
const DONE_MARKER = '/tmp/promus-relaunch-done'
const FAIL_MARKER = '/tmp/promus-relaunch-failed'
...
export function buildGatewayRelaunchScript(
  ...
  const innerPath = '/tmp/promus-relaunch-inner.sh'
