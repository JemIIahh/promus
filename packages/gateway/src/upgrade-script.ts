/**
 * In-place upgrade script template. Used by `promus upgrade` (default flow,
 ...
 *
 * Why in-place: per `feedback-promus-is-unsealed-currently.md`, our Phase 11
 * deployment is unsealed (no image-hash attestation, no TDX-bound signer).
 * Heavy container swap buys no real attestation, only ~0.9 ETH testnet burn
 ...
 * npm → git) requires `promus upgrade --reprovision`.
 *
 ...
 *  - Progress via two files: `/tmp/promus-upgrade-progress.log` (tail-able),
 *    `/tmp/promus-upgrade-done` (success only, contains harness pid).
 *  - Failure markers in `/tmp/promus-upgrade-failed` for each step's distinct
 *    failure keyword.
 */
...
export interface BuildUpgradeScriptOpts {
  ...
  /**
   * Public git URL of promus. Defaults to canonical hackathon repo. Re-applied
...
const PROGRESS_LOG = '/tmp/promus-upgrade-progress.log'
const DONE_MARKER = '/tmp/promus-upgrade-done'
const FAIL_MARKER = '/tmp/promus-upgrade-failed'
...
function buildRestartLines(opts: BuildUpgradeScriptOpts, gatewayLaunchCmd: string): string[] {
  ...
  return [
    ...
    'rm -f "$HOME/.promus/locks/"*.lock 2>/dev/null || true',
    '',
...
function buildGitInnerScript(opts: BuildUpgradeScriptOpts): string {
  ...
  const installLines = [
    `cd "$HOME/promus" || { echo "promus-dir-missing" > ${FAIL_MARKER}; exit 20; }`,
...
function buildNpmInnerScript(opts: BuildUpgradeScriptOpts): string {
  ...
  const installLines = [
    ...
    `retry 'promus install' bun add -g ${shQuote(`promus@${opts.packageVersion}`)} || { echo "promus-install-failed" > ${FAIL_MARKER}; exit 21; }`,
...
export function buildUpgradeScript(opts: BuildUpgradeScriptOpts): BuildUpgradeScriptResult {
  ...
  const innerPath = '/tmp/promus-upgrade-inner.sh'
