...
describe('buildUpgradeScript', () => {
  ...
  test('outer wrapper is `bash -c` with base64 inner; no shell metachars in payload', () => {
    ...
    expect(script).toContain('nohup bash /tmp/promus-upgrade-inner.sh')
  ...
  test('inner subshell carries git fetch + checkout + bun install + harness restart', () => {
    ...
    expect(inner).toContain('cd "$HOME/promus"')
    ...
    expect(inner).toContain('pkill -f promus-gateway')
    expect(inner).toContain('nohup bun "$HOME/promus/packages/gateway/bin/promus-gateway"')
    expect(inner).toContain(`echo "promus-gateway-pid=$HARNESS_PID" > ${UPGRADE_DONE_MARKER}`)
  })
  ...
  test('clears stale agent locks after killing prior harness, before relaunch', () => {
    ...
    const killIdx = inner.indexOf('pkill -f promus-gateway')
    const lockIdx = inner.indexOf('rm -f "$HOME/.promus/locks/"*.lock')
    const launchIdx = inner.indexOf('nohup bun "$HOME/promus/packages/gateway/bin/promus-gateway"')
  ...
  test('inner subshell writes fail marker on each step failure', () => {
    const inner = decodeInner()
    expect(inner).toContain('promus-dir-missing')
  ...
  test('exposes UPGRADE_SUCCESS_MARKER_PREFIX matching bootstrap (callers grep done file)', () => {
    expect(UPGRADE_SUCCESS_MARKER_PREFIX).toBe('promus-gateway-pid=')
  })
  ...
  test('uses $HOME/promus (matches bootstrap clone target; daytona user has no /opt sudo)', () => {
    const inner = decodeInner()
    expect(inner).toContain('cd "$HOME/promus"')
  ...
  describe('mode=npm', () => {
    ...
    test('inner subshell does `bun add -g` instead of git fetch+checkout+bun install', () => {
      ...
      expect(inner).toContain("bun add -g 'promus@0.21.15'")
    ...
    test('inner subshell launches gateway from global bin', () => {
      ...
      expect(inner).not.toContain('nohup bun "$HOME/promus/packages/gateway/bin/promus-gateway"')
    })
  ...
  test('default mode is npm when not specified (flipped in v0.21.20; ~10x faster cold start)', () => {
    ...
    expect(inner).toContain('upgrade-start (mode=npm)')
    expect(inner).toContain("bun add -g 'promus@0.21.20'")
    expect(inner).not.toContain('git fetch')
  })
})
