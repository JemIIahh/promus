/**
 * Bootstrap script template for first-cold-start of an promus harness inside
 ...
 *
 *  - `/tmp/promus-bootstrap-progress.log` (tail-able, line-by-line stages)
 *  - `/tmp/promus-bootstrap-done` (created only on full success, contains harness pid)
 *
...
export interface BuildBootstrapScriptOpts {
  ...
  /**
   * Public git URL of promus. Defaults to the canonical hackathon repo.
  ...
  /**
   * GitHub PAT for cloning private promus repos. Embedded in clone URL as
...
const PROGRESS_LOG = '/tmp/promus-bootstrap-progress.log'
const DONE_MARKER = '/tmp/promus-bootstrap-done'
const FAIL_MARKER = '/tmp/promus-bootstrap-failed'
 ...
 * `BootstrapProgressBox`). Strings are prefixes — the apt/promus/chrome rows
...
export const BOOTSTRAP_STAGE_MARKERS = {
  ...
  promusInstall: 'installing promus',
...
function buildLaunchLines(opts: BuildBootstrapScriptOpts, gatewayLaunchCmd: string): string[] {
  ...
  return [
    'mkdir -p "$HOME/promus-logs" "$HOME/workspace"',
    '',
...
function buildGitInnerScript(opts: BuildBootstrapScriptOpts, aptList: string): string {
  ...
  const installLines = [
    ...
    `echo "STAGE: ${BOOTSTRAP_STAGE_MARKERS.promusInstall} (git ${opts.ref})"`,
...
function buildNpmInnerScript(opts: BuildBootstrapScriptOpts, aptList: string): string {
  ...
  const installLines = [
    ...
    `echo "STAGE: ${BOOTSTRAP_STAGE_MARKERS.promusInstall} (${opts.packageVersion})"`,
    // Install promus from npm. `bun add -g <pkg>@<exact-version>` is idempotent
...
export function buildBootstrapScript(opts: BuildBootstrapScriptOpts): BuildBootstrapScriptResult {
  ...
  const innerPath = '/tmp/promus-bootstrap-inner.sh'
