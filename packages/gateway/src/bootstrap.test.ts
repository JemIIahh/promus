...
describe('buildBootstrapScript', () => {
  ...
  describe('mode=npm', () => {
    ...
    test('inner subshell does `bun add -g @s0nderlabs/promus@<version>` instead of git clone', () => {
      const inner = decodeInner(npmOpts)
      expect(inner).toContain("bun add -g '@s0nderlabs/promus@0.21.15'")
  ...
  test('default mode is npm when not specified (flipped in v0.21.20; ~10x faster cold start)', () => {
    ...
    expect(inner).toContain('bootstrap-start (mode=npm)')
    expect(inner).toContain("bun add -g '@s0nderlabs/promus@0.21.20'")
