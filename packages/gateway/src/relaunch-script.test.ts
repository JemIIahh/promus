...
describe('buildGatewayRelaunchScript', () => {
  ...
  test('outer wrapper is `bash -c` with base64 inner', () => {
    ...
    expect(script).toContain('nohup bash /tmp/promus-relaunch-inner.sh')
  ...
  test('auto-detects bootstrap mode by probing filesystem (no hard $HOME/promus check)', () => {
    ...
    expect(inner).toContain('echo "promus-not-installed"')
  })
  ...
  test('legacy "promus-dir-missing" failure keyword removed in favor of "promus-not-installed"', () => {
    const inner = decodeInner()
    expect(inner).not.toContain('promus-dir-missing')
    expect(inner).toContain('promus-not-installed')
  })
