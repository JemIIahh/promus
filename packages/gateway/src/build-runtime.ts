...
export interface BuildRuntimeOpts {
   ...
   * `~/.promus/config.ts` know where to write back. The harness creates an
   ...
   * `cd "$PROMUS_DIR"` (= `$HOME/promus` on Daytona) before launching the
  ...
  /** v0.24.11: same as `onAutoTriggerInbox` but for PromusMarket events. */
  onAutoTriggerMarket?: () => void
}

/** Either brain backend — both implement the methods the harness uses
 *  (init / infer / refreshUserContext / clearChannel). */
type RuntimeBrain = OGComputeBrain | AnthropicBrain
 ...
 * registered (e.g. "specter" for `specter.promus.0g`); falls back to a
...
/**
 * Construct the full promus runtime (tools, brain, plugins, listeners, sync)
 ...
 */
export async function buildPromusRuntime(opts: BuildRuntimeOpts): Promise<BuiltRuntime> {
  ...
  // `promus profile init` ships a fresh key via /admin/profile-key.
  ...
  if (pluginNames.includes('onchain')) {
    onchain = {
      ...
      // 4-char `0x04` prefix to match the body form used in .promus.0g records.
  ...
  // path (`~/.promus/agents/<id>/pairing`) so codes generated by the daemon
  // are visible to `promus pairing approve` running on the operator's machine.
  // The daemon's tmp-scratch agentDir (under TMPDIR/promus-gateway) is NOT
  ...
  const delegateFactory: import('promus-core').DelegateBrainFactory = async ({
    ...
    const subBrain: RuntimeBrain = useAnthropic
  ...
  //     `~/.promus/agents/<id>/memory` snapshot and ignores the operator's
  ...
  //     until `promus profile init` ships a key via /admin/profile-key.
  const sync = new MemorySyncManager({
    ...
    // ${TMPDIR}/promus-gateway/<id>/... (TMPDIR is volatile across boots) while
    // the default in MemorySyncManager resolves under ~/.promus/agents/<id>/...
  ...
  const brainOpts = {
    ...
    onToolCall: async (call: { id: string; name: string; args: unknown }) => {
      ...
      // /console + promus-launch dashboards hold persistent SSE
  ...
  const brain: RuntimeBrain = useAnthropic
  ...
  if (!useAnthropic && economyEnabled && config.brain?.provider) {
    ...
    autoTopup = new AutoTopupManager(
      ...
      },
      {
        agentAddress,
        publicClient: viemClients.publicClient,
        getBrokerLedger: async () => (brain instanceof OGComputeBrain ? brain.getLedger() : null),
  ...
  // operator runs `promus profile init` against a sandbox endpoint and the
