...
import { type BuiltRuntime, buildPromusRuntime } from './build-runtime'
...
export class RealRuntime implements RuntimeAdapter {
  ...
  #network: PromusNetwork | null = null
  ...
  async start(opts: {
    ...
    const runtime = await buildPromusRuntime({
      config: opts.config,
    ...
    // we plumb startAll outcomes; right now buildPromusRuntime swallows them.
    ...
    // 'active' as registered + started; if buildPromusRuntime swallowed a start
  ...
  async runChatTurn(input: ChatTurnInput): Promise<ChatTurnResult> {
    ...
    // Per-turn sync flush is BACKGROUND. Chain anchor on Arbitrum mainnet takes
