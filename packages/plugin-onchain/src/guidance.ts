...
- Balance + identity: \`chain.balance\` with no args returns native + every ERC-20 the agent has ever held (Transfer-event discovery, no curated list). Pass \`token\` for a single asset or \`address\` to inspect another wallet. \`account.info\` bundles wallet + iNFT + brain provider + recent activity AND the agent's own \`.promus.0g\` subname, A2A pubkey, and the canonical promus singleton addresses (inbox/market/agentNFT). Call it before answering identity / "who are you" / "what's your pubkey" questions instead of guessing.
...
- Promus singletons (CREATE2-deterministic, same address on testnet and mainnet, introspect via \`chain.contract\`):
  - \`PromusInbox\` at \`0xcd92844cc0ec6Be0607B330D4BaCC707339f2589\`: singleton A2A encrypted message emitter (ECIES, ERC-7857 inbox, Phase 7).
  - \`PromusMarket\` at \`0x3ebD21f5dd67acDeF199fACF28388627212bA2aB\`: ERC-8183 native-0G fixed-price escrow marketplace (Phase 8).
  - \`PromusAgentNFT\` at \`0x9e71d79f06f956d4d2666b5c93dafab721c84721\`: ERC-7857 iNFT identity registry (Phase 4).
  When the user asks for "metadata for X contract" / "tell me about PromusInbox" / "what does PromusMarket do", call \`chain.contract\` on the address above (NOT \`shell.run\` to grep the codebase, NOT \`memory.read\`).
