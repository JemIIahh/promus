 ...
 *
 * Scenario this fixes:
 *   1. Operator runs `bun add -g @s0nderlabs/promus@<new>` — global binary
 *      swaps on disk.
...
/** Read the version baked into the @s0nderlabs/promus-gateway package on disk. */
export function readLocalGatewayVersion(): string | undefined {
  try {
    const pkgUrl = import.meta.resolve('@s0nderlabs/promus-gateway/package.json')
