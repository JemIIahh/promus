 ...
 * body is the prompt that promus inlines when the command/agent fires.
 */

export interface ClaudeCommand {
  /** `<plugin>:<name>` (promus drops the marketplace prefix; the cmd surface is flat). */
  id: string
  ...
  path: string
  /** Body without frontmatter; this is the prompt template promus inlines. */
  body: string
...
export interface ClaudeAgent {
  ...
  name: string
  description: string
  /** Optional model hint from frontmatter (e.g. `sonnet`). promus ignores it (uses configured brain). */
  model?: string
  path: string
  body: string
  source: { marketplace: string; plugin: string; version: string }
