...
import type { PromusConfig } from '../config'
import { EventQueue, listeners, newEventId, routeLoop } from '../events'
import type { PromusEvent } from '../events/types'
...
export class Runtime {
  ...
  async fire(event: Omit<PromusEvent, 'id' | 'ts'>): Promise<string> {
    const ev: PromusEvent = { ...event, id: newEventId(), ts: Date.now() }
    await this.activity?.append({
      ts: ev.ts,
      kind: 'wake',
