 ...
 * prompting Touch ID. Written once via `promus gateway start` after an
 * interactive Touch ID unlock; read by the daemon at boot.
 *
 * Security model:
 *  - File at `~/.promus/agents/<id>/.operator-session` with permission 0600.
 ...
 *
 * Used by `promus gateway start` and TUI auto-spawn to decide whether the
 ...
 *
 * The closing the gap on the v0.21.12 regression where `promus gateway start`
 ...
 * Convenience composer used by `promus gateway start`.
 */
