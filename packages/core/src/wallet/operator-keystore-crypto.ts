...
export async function encryptAgentKey(opts: {
   ...
   * `signer.signTypedData` entirely. Used by `promus init` to share the same
...
export async function decryptAgentKey(opts: {
   ...
   * prior interactive `promus gateway start` derives the key once via the
  ...
  if (opts.keystore.version !== OPERATOR_KEYSTORE_VERSION) {
    throw new Error(
      `Unsupported operator keystore version: ${opts.keystore.version} (expected ${OPERATOR_KEYSTORE_VERSION}). For v1 (passphrase) keystores, run \`promus migrate-keystore\` first.`,
    )
  }
 ...
 * `~/.promus/agents/<id>/telegram-secrets.encrypted`.
 *
 ...
 * Used by `promus restore` and `promus migrate-keystore` to branch between the
