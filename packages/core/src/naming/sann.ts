 ...
 */
export const SANN_SUFFIX = '.promus.0g' as const
...
/** Compute the SANN node for `<label>.promus.0g`. */
export function subnameNode(label: string): Hex {
  return sannNamehash(SANN_ADDRESSES.tldIdentifier, '0g', ['promus', label])
}

/** Read the registry owner of any SANN node. Shared by SannClient + PromusRegistrarClient. */
...
/**
 * Resolve a `<label>.promus.0g` subname to the address text record published
...
export class SannClient {
  ...
  private readonly promusNode: Hex
  ...
  constructor(opts: SannClientOpts) {
    ...
    this.promusNode = sannNamehash(SANN_ADDRESSES.tldIdentifier, '0g', ['promus'])
  }

  /**
   * Issue `<label>.promus.0g` to `owner`. Caller must be the registry owner
   * of `promus.0g`. Calls `SidRegistry.setSubnodeOwner(promusNode, labelHash, owner)`
  ...
  async reclaimSubname(label: string, owner: Address): Promise<Hex> {
    ...
    return await this.walletClient.writeContract({
      ...
      args: [this.promusNode, labelHash, owner],
      chain: this.chain,
