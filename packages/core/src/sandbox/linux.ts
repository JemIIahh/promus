 ...
 *   - writable bind of /tmp (promus-* dirs land there)
 ...
 *   - --unshare-all --share-net keeps network reachable so promus can still
 *     hit 0G RPC, the indexer, etc.
 *   - --die-with-parent kills the child if promus crashes, no zombies
 *   - --new-session puts the child in its own session (Ctrl-C from promus
...
export function buildBwrapArgs(opts: SandboxBackendOpts): string[] {
  ...
  // Writable subdirs: agentDir + workspaceRoot + /tmp (for promus-* test dirs)
  ...
  // Namespace isolation: unshare everything except network (promus needs network
  ...
  args.push('--die-with-parent') // child dies if promus dies
  args.push('--new-session') // Ctrl-C from promus doesn't kill the child directly
...
export class LinuxBubblewrapBackend implements SandboxBackend {
  ...
  envHint(): SandboxEnvHint {
    return {
      ...
      workspaceMount: null,
      scope:
        'shell.run, code.execute, shell.process_start are wrapped in a bubblewrap profile; writes outside agentDir + cwd + /tmp/promus-* are denied',
    }
  }
