 ...
 *
 * Phase 9.5 (Apr 28 2026 incident response). Promus's limbs run on the operator's
...
/**
 * Mode selector. Lives under `sandbox.mode` in `~/.promus/config.ts`.
 *
 ...
 *  - `agentDir`: write-allowed (promus writes activity log, mcp debug, etc.).
 *  - `workspaceRoot`: write-allowed (where the operator launched promus from;
...
export interface SandboxBackend {
  ...
  /** Optional cleanup (kill long-lived containers, remove temp files). Called on promus exit. */
