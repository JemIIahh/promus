 ...
 *    promus's RPC/storage/compute/WC traffic to escape the container).
 *  - Cold-start cost ~1s on the FIRST tool call after promus boot (longer if
 ...
 * still runs on host. A future bundle would re-exec all of promus inside the
...
export interface DockerBackendOpts extends SandboxBackendOpts {
   ...
   * unlimited (runtime default). Hermes default is 1; promus leaves UNSET so
...
export class DockerBackend implements SandboxBackend {
  ...
  private async startContainer(): Promise<string> {
    ...
    const runArgs: string[] = ['run', '-d', '--rm', '--label', 'promus-sandbox=1', ...HARDENING_ARGS]
