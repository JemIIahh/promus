...
describe('buildSeatbeltProfile', () => {
  test('emits a deny-default profile that explicitly allows agentDir + workspaceRoot', () => {
    const profile = buildSeatbeltProfile({
      agentDir: '/Users/test/.promus/agents/abc',
      workspaceRoot: '/Users/test/Documents/proj',
      homedir: '/Users/test',
    })
    expect(profile).toContain('(deny default)')
    expect(profile).toContain('(allow file-write* (subpath "/Users/test/.promus/agents/abc"))')
  ...
  test('explicitly denies credential paths for both reads and writes', () => {
    ...
    expect(profile).toContain('(deny file-write* (subpath "/Users/test/.promus"))')
  ...
  test('allows /tmp/promus-* + /var/folders for temp dirs', () => {
    const profile = buildSeatbeltProfile({
      agentDir: '/a',
      workspaceRoot: '/b',
      homedir: '/h',
    })
    expect(profile).toContain('(allow file-write* (regex #"^/tmp/promus-"))')
    expect(profile).toContain('(allow file-write* (regex #"^/private/tmp/promus-"))')
  ...
  test('respects extraWriteAllow + extraWriteDeny', () => {
    const profile = buildSeatbeltProfile({
      ...
      workspaceRoot: '/b',
      homedir: '/h',
      extraWriteAllow: ['/tmp/promus-test-sandbox-XYZ'],
      extraWriteDeny: ['/Users/h/Documents/sensitive'],
    })
    expect(profile).toContain('(allow file-write* (subpath "/tmp/promus-test-sandbox-XYZ"))')
  ...
  test('re-allows agentDir AFTER the broad ~/.promus deny so promus state stays writable', () => {
    const profile = buildSeatbeltProfile({
      agentDir: '/Users/test/.promus/agents/abc',
      workspaceRoot: '/w',
      homedir: '/Users/test',
    ...
    const agentAllowIdx = profile.lastIndexOf(
      '(allow file-write* (subpath "/Users/test/.promus/agents/abc"))',
    )
    const promusDenyIdx = profile.indexOf('(deny file-write* (subpath "/Users/test/.promus"))')
    expect(agentAllowIdx).toBeGreaterThan(-1)
    expect(promusDenyIdx).toBeGreaterThan(-1)
    expect(agentAllowIdx).toBeGreaterThan(promusDenyIdx)
  })
})
...
describe('MacOSSandboxExecBackend', () => {
  ...
  test('constructs with valid opts on darwin', () => {
    const b = new MacOSSandboxExecBackend({
      agentDir: '/tmp/promus-test-agent',
      workspaceRoot: '/tmp',
    ...
    expect(profile).toContain('(deny default)')
    expect(profile).toContain('(allow file-write* (subpath "/tmp/promus-test-agent"))')
  })
...
describe('buildBwrapArgs (Linux profile)', () => {
  test('binds agentDir + workspaceRoot writable', () => {
    const args = buildBwrapArgs({
      agentDir: '/home/u/.promus/agents/abc',
      workspaceRoot: '/home/u/proj',
      homedir: '/home/u',
    })
    expect(args).toContain('--bind')
    expect(args.join(' ')).toContain('/home/u/.promus/agents/abc /home/u/.promus/agents/abc')
  ...
  test('respects extraWriteAllow for sandbox dirs', () => {
    ...
    expect(args.join(' ')).toContain(
      '--bind /tmp/promus-test-sandbox-XYZ /tmp/promus-test-sandbox-XYZ',
    )
  })
