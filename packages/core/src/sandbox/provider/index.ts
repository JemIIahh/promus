 ...
 *
 * If the provider rotates IPs or domain, update this constant. Promus callers
 ...
 * init wizard's cost summary and by `promus topup --sandbox` for the runway
 ...
 * `promus init --target sandbox` provisions a fresh container. Mirrors the
...
export interface BuildSandboxEndpointOpts {
  sandboxId: string
  /** Default 8080 — promus harness binds 8080 inside the container. */
  port?: number
}
