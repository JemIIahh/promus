 ...
 *     - agentDir (promus state)
 *     - workspaceRoot (the cwd promus was launched from; fs.write authorized
 *       through the modal lands here)
 *     - /tmp/promus-* (promus's own temp scratch — code.execute snippets, etc.)
 *     - /private/tmp/promus-* (macOS canonical /tmp)
 ...
 *     - $HOME/.config/gcloud
 *     - $HOME/.promus (the broader promus state tree — only the agent's own
 *       agentDir is allowed; brain shouldn't rewrite ~/.promus/config.ts)
 *
 *   NETWORK: allow* (promus legitimately needs 0G RPC, indexer, compute,
 ...
 * Newlines also break the parser. Promus paths come from process.cwd() and
...
export function buildSeatbeltProfile(opts: SeatbeltProfileOpts): string {
  ...
  const allowSubpaths = [
    ...
    `(allow file-write* (regex #"^/tmp/promus-"))`,
    `(allow file-write* (regex #"^/private/tmp/promus-"))`,
  ...
  const denySubpaths = [
    ...
    `(deny file-write* (subpath "${home}/.promus"))`,
  ...
  // The agentDir is under ~/.promus/agents/<id>/, and we deny ~/.promus broadly,
  // so we MUST re-allow agentDir AFTER the deny to keep promus's own state
...
;; Network — broad. Promus needs 0G RPC, indexer, compute, WC relay, plus
...
;; Re-allow agentDir AFTER the ~/.promus broad deny so promus's own state is
