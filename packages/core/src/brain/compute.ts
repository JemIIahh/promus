/**
 * Compute-backed brain. Upstream `@0glabs/0g-serving-broker` requires an
 * ethers Wallet/Signer — this file is the ONLY place in promus where ethers
...
export class OGComputeBrain implements Brain {
  ...
  /** Live ledger balance in ETH (the prepaid compute credit). null on failure. */
  ...
  async infer(input: BrainInferInput): Promise<BrainTurn> {
    ...
    while (true) {
      ...
      if (!resp.toolCalls.length) {
        // qwen3.6-plus on the broker sometimes returns its safety-filter
  ...
  private async callCompletion(messages: BrainMessage[], signal?: AbortSignal): Promise<BrainTurn> {
    ...
    // The broker (DashScope) rejects an empty tools array (`[] is too short`).
    ...
    if (!resp.ok) {
      ...
      // Translate the provider's "insufficient balance" HTTP 400 into a
      // typed error so dispatchers (TUI + TG) can render an actionable
      // message ("promus topup --compute N") instead of the raw HTTP body.
...
/**
 * Detect the broker's safety-filter reject ("An error occurred while
...
/**
 * Thrown when the provider rejects the request with HTTP 400 because the
 ...
 *    insufficient balance: your locked balance is X ETH, but the required
 *    minimum is Y ETH (breakdown: minimum reserve A ETH + unsettled fees
 *    B ETH + current request fee C ETH). Please add more"
 */
export class LedgerInsufficientError extends Error {
  /** Locked balance in the provider sub-account, in ETH (string for precision). */
  readonly availableOg: string
  /** Required minimum, in ETH. */
  readonly requiredOg: string
  /** required − available, in ETH. */
  ...
  constructor(opts: {
    ...
    super(
      `Compute ledger sub-account short by ${opts.shortfallOg} ETH (provider ${opts.providerAddress.slice(0, 10)}…, locked ${opts.availableOg} of ${opts.requiredOg} required). Topup with: promus topup --compute 2`,
    )
...
const LEDGER_ERROR_RE =
  /insufficient balance: your locked balance is ([\d.]+)\s*ETH, but the required minimum is ([\d.]+)\s*ETH/i
