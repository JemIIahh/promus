...
import { type PromusNetwork, NETWORK_RPC } from '../config'

/**
 * Compute ledger helpers. The ledger is a prepaid settlement account for
 * inference: deposit ETH up front, sign EIP-712 vouchers per request, provider
 * settles periodically. No chain tx per inference call.
 *
 * Contract minimum deposit is 3 ETH. Calls here use `ethers` because the
...
export interface OpenLedgerOpts {
  ...
  privkeyHex: Hex
  /** Initial deposit in ETH (floating point). Contract minimum 3. */
...
export function setBrokerFactoryForTests(
  factory: ((network: PromusNetwork, privkeyHex: Hex) => Promise<Broker>) | null,
): void {
  brokerFactory = factory
  brokerCache.clear()
}

let brokerFactory: ((network: PromusNetwork, privkeyHex: Hex) => Promise<Broker>) | null = null

async function makeBroker(network: PromusNetwork, privkeyHex: Hex): Promise<Broker> {
...
async function defaultBrokerFactory(network: PromusNetwork, privkeyHex: Hex): Promise<Broker> {
...
/**
 * Open or top up the agent's compute ledger. If no ledger exists, calls
...
/**
 * Top up the existing ledger by `amount` ETH. Agent EOA pays gas and the
...
/**
 * Transfer `amount` ETH from the agent's main ledger (already opened + funded
 * via depositToLedger) into a specific provider sub-account. The compute
 * broker SDK requires per-provider sub-accounts; the init wizard only seeds the
...
export async function transferFundToProvider(opts: {
  ...
  provider: Address
  /** Amount in ETH (floating point), converted to wei internally. */
  amount: number
}): Promise<void> {
  const broker = await makeBroker(opts.network, opts.privkeyHex)
...
/**
 * Withdraw `amount` ETH from the main ledger account back to the agent EOA.
...
/**
 * Compute LedgerManager contract addresses, mirrored from the upstream
 ...
 */
const LEDGER_MANAGER_ADDRESS: Record<PromusNetwork, Address> = {
  '0g-mainnet': '0x2dE54c845Cd948B72D2e32e39586fe89607074E3',
  '0g-testnet': '0xE70830508dAc0A97e6c087c75f402f9Be669E406',
  // Compute-only ledger. Never read on Arbitrum (the Anthropic brain has no
  // on-chain compute ledger). Placeholder until the 0G brain is removed.
