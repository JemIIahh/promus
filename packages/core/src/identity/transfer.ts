 ...
 * ownership change. Pairs with `PromusAgentNFT.iTransferFrom` in the contract.
 *
...
export interface TransferProofPreimageArgs {
  ...
  proofNonce: Hex
  /** The PromusAgentNFT contract address (`address(this)` in Solidity). */
...
export interface BuildTransferHashesArgs {
   ...
   * fetches via `PromusAgentNFTReader.getIntelligentData(tokenId)`.
   */
