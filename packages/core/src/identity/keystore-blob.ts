...
import { createStorage, downloadBlobByRoot } from '../storage'
import {
...
export async function uploadAndAnchorKeystore(opts: {
  ...
  const storage = createStorage({ network: opts.network, privkeyHex: opts.agentPrivkey })
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      if (!r.startsWith('0x') || r.length !== 66) {
        throw new Error(
          `Storage returned a content hash that doesn't fit bytes32 (${r.length} chars); cannot anchor to iNFT slot`,
        )
      }
      rootHash = r
      break
  ...
  if (!rootHash) {
    throw new Error(
      `Storage upload failed after 3 attempts. Last error: ${lastErr?.message ?? 'unknown'}`,
    )
  }
  const client = new PromusAgentNFTClient({
