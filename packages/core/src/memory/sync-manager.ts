...
import { PromusAgentNFTClient, PromusAgentNFTReader } from '../identity/contract'
import {
...
export class MemorySyncManager {
  ...
  private readonly nft: PromusAgentNFTClient
  ...
  constructor(private readonly opts: MemorySyncManagerOpts) {
    this.storage = createStorage({ network: opts.network, privkeyHex: opts.agentPrivkey })
    this.nft = new PromusAgentNFTClient({
