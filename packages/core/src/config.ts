...
export type PromusNetwork = '0g-mainnet' | '0g-testnet' | 'arbitrum-sepolia' | 'robinhood-testnet'

export type PromusPlugin = 'onchain' | 'comms' | 'system' | 'telegram'
...
export type PromusConfigInput = Partial<PromusConfig> & Pick<PromusConfig, 'network'>

const DEFAULT_CONFIG: Omit<PromusConfig, 'network' | 'storage'> = {
...
export function defineConfig(input: PromusConfigInput): PromusConfig {
  return {
    ...DEFAULT_CONFIG,
...
export const NETWORK_RPC: Record<PromusNetwork, string> = {
  '0g-mainnet': 'https://evmrpc.0g.ai',
  '0g-testnet': 'https://evmrpc-testnet.0g.ai',
  // Arbitrum Sepolia testnet (Nitro rollup). Faucets: alchemy.com/faucets/arbitrum-sepolia.
  'arbitrum-sepolia': 'https://sepolia-rollup.arbitrum.io/rpc',
  // Robinhood Chain testnet (Arbitrum Orbit L2, permissionless). ETH gas.
  // Faucet: faucet.testnet.chain.robinhood.com. Explorer: Blockscout.
  'robinhood-testnet': 'https://rpc.testnet.chain.robinhood.com',
}

export const NETWORK_CHAIN_ID: Record<PromusNetwork, number> = {
  '0g-mainnet': 16661,
  '0g-testnet': 16602,
...
export function networkFromChainId(id: number): PromusNetwork | null {
  return (Object.entries(NETWORK_CHAIN_ID).find(([, cid]) => cid === id)?.[0] ??
    null) as PromusNetwork | null
}
