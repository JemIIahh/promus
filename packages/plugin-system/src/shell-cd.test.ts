...
describe('shell.cd', () => {
  it('updates the shared cwd state to a valid absolute path', async () => {
    const dir1 = await mkdtemp(join(tmpdir(), 'promus-cd-a-'))
    const dir2 = await mkdtemp(join(tmpdir(), 'promus-cd-b-'))
    try {
  ...
  it('resolves relative paths against the current cwd', async () => {
    const root = await mkdtemp(join(tmpdir(), 'promus-cd-rel-'))
  ...
  it('refuses paths outside permitted scope (~/.ssh)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'promus-cd-ssh-'))
    try {
      const cwd = new WorkingDirState(root)
      const cd = makeShellCd({ cwd, agentDir: '/tmp/fake-agent-dir' })
  ...
  it('refuses paths under the agent state tree', async () => {
    const root = await mkdtemp(join(tmpdir(), 'promus-cd-agentdir-'))
  ...
  it('refuses non-existent paths', async () => {
    const root = await mkdtemp(join(tmpdir(), 'promus-cd-nx-'))
    try {
      const cwd = new WorkingDirState(root)
      const cd = makeShellCd({ cwd, agentDir: '/tmp/fake-agent-dir' })
  ...
  it('refuses paths that point to a regular file', async () => {
    const root = await mkdtemp(join(tmpdir(), 'promus-cd-file-'))
  ...
  it('shell.run sees the new cwd after shell.cd', async () => {
    const dir1 = await mkdtemp(join(tmpdir(), 'promus-cd-shr-a-'))
    const dir2 = await mkdtemp(join(tmpdir(), 'promus-cd-shr-b-'))
