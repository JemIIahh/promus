...
const plugin: NativePlugin = {
  ...
  register: ctx => {
    ...
    const inbox = new PromusInboxClient({
    ...
    // Phase 8: PromusMarket escrow. Optional — only registers if the harness
    // injected `comms.marketAddress` (chat.tsx wiring).
    if (comms.marketAddress) {
      const market = new PromusMarketClient({
...
export interface CommsRuntimeContext {
  ...
  /** Optional: PromusMarket address. When set, plugin registers 9 market.* limbs + listener. */
