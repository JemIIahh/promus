...
import { type PromusMarketClient, JOB_STATUS, JOB_STATUS_LABEL, type Job } from './market'
import type { PubkeyResolver } from './pubkey-resolver'
import { resolveAddrOrName } from './tools'

export interface MarketDeps {
  market: PromusMarketClient
...
const CreateJobSchema = z.object({
  provider: z
    .string()
    .min(1)
    .describe('Counterparty doing the work: an .promus.0g name, raw 0x address, or contact label.'),
  amount: z
    .string()
    .min(1)
    .describe('Escrow amount in ETH (e.g. "0.05" or "1.5"). Minimum 0.001 ETH.'),
  description: z
    .string()
    .min(1)
    .max(2000)
...
export function makeMarketCreateJob(deps: MarketDeps): ToolDef<CreateJobArgs> {
  return {
    ...
    description:
      'Create + fund a fixed-price escrow job for another promus. Atomic: one tx funds the job, locks ETH in PromusMarket, emits JobCreated. Provider does the work, calls market.markDone, then you call market.acceptResult or market.dispute. 5% protocol fee at settle. 24h acceptance window after markDone, 7-day max lifetime.',
    ...
    handler: async args => {
      try {
        ...
        if (!r) {
          return {
            ok: false,
            error: `unrecognized provider: ${args.provider}. Use a .promus.0g name, 0x address, or contact label.`,
          }
        }
...
const ProposeSplitSchema = z.object({
  ...
  buyerAmount: z
    .string()
    .describe(
      'Amount in ETH the buyer should receive in this split. Sum with providerAmount must equal job amount.',
    ),
  providerAmount: z.string().describe('Amount in ETH the provider should receive in this split.'),
})
...
export function makeMarketGetJob(deps: MarketDeps): ToolDef<JobIdArgs> {
  return {
    name: 'market.getJob',
    description:
      'Read-only inspector for a job. Returns buyer, provider, amount in ETH, status (funded/done/disputed/settled), createdAt, doneAt timestamps, and the descriptionHash.',
