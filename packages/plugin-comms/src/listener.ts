...
/**
 * Listener for PromusInbox.Message events targeting the agent's own EOA.
 * Boot sequence:
...
export interface ListenerOpts {
   ...
   * blocks (~1 min on Arbitrum's ~0.25s block time, well within RPC log-window
...
export class A2AListener {
  ...
  private async handleEvent(ev: InboxMessageEvent): Promise<void> {
    ...
    //    re-fetch every spillover blob from IPFS (HTTP, retried) AND
