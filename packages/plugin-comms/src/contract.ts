...
export interface InboxClientOpts {
  /** Deployed PromusInbox singleton on the agent's network. */
  address: Address
...
export class PromusInboxClient {
  ...
  async send(to: Address, payload: Hex, dataHash: Hex): Promise<Hex> {
    if (!this.walletClient) {
      if (!this.privkeyHex) {
        throw new Error('PromusInbox.send requires walletClient or privkeyHex')
      }
      // Caller didn't supply a walletClient; we can't synthesize one without
      // a chain reference. Refuse rather than guess.
      throw new Error(
        'PromusInbox.send: walletClient missing. Build a WalletClient with the agent privkey + chain.',
      )
    }
