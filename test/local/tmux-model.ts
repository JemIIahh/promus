/**
 * tmux driver for `promus model`. The picker is interactive — we just verify
...
await runTmuxTest(
  ...
  async s => {
    ...
    console.log('[ok] promus model — catalog fetched + picker rendered')

    // Cancel without picking so the config doesn't change.
    await sendKeys(s, 'C-c')
    await sleep(2_000)
    console.log('[ok] promus model — cancelled cleanly')
  },
  'bun packages/cli/bin/promus model',
)
