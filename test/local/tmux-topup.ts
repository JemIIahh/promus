/**
 * tmux driver for `promus topup --agent <amount>`. Verifies the operator
...
await runTmuxTest(`promus-topup-${process.pid}`, async s => {
  const { exit, pane } = await runOneShot(
    s,
    'bun packages/cli/bin/promus topup --agent 0.001',
    120_000,
  )
  ...
  console.log('[ok] promus topup --agent 0.001 completed')
})
