/**
 * tmux driver for `promus inspect`. Each invocation runs in its own tmux
...
async function main(): Promise<void> {
  ...
  const raw = await oneShot('raw', `${PROMUS} inspect --raw`, 90_000)
  ...
  const slot = await oneShot('slot', `${PROMUS} inspect --raw --slot identity`, 30_000)
  if (slot.exit !== 0) {
  ...
  const badSlot = await oneShot('badslot', `${PROMUS} inspect --slot bogus`, 10_000)
  ...
  // Mode D: foreign iNFT ref. Pull active iNFT from `promus status`, pass back
  // positionally. Should annotate as foreign and skip operator unlock.
  const status = await oneShot('status', `${PROMUS} status`, 15_000)
  ...
  if (!m) throw new Error(`could not parse iNFT from promus status\n${status.pane.slice(-1500)}`)
  const [, tokenId, contract, network] = m
  const ref = `${network}:${contract}:${tokenId}`
  const foreign = await oneShot('foreign', `${PROMUS} inspect ${ref}`, 90_000)
  if (foreign.exit !== 0) {
  ...
  const j = await oneShot('json', `${PROMUS} inspect --raw --json --slot identity`, 30_000)
  ...
  console.log('\n[ok] promus inspect — 5 modes verified end-to-end')
}
