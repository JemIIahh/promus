/**
 * tmux driver for `promus sync` (force-flush memory + activity-log to 0G).
 *
 ...
 * hint already persisted in ~/.promus/config.ts.
 */
import { runOneShot, runTmuxTest } from './_tmux'

await runTmuxTest(`promus-sync-${process.pid}`, async s => {
  const { exit, pane } = await runOneShot(s, 'bun packages/cli/bin/promus sync', 180_000)
  if (exit !== 0) throw new Error(`sync exited with code ${exit}`)
  if (/anchored \d+ slot\(s\)|tx:\s*https:\/\/.*tx\/0x[0-9a-f]+/i.test(pane)) {
    console.log('[ok] promus sync — chain anchor created')
  } else if (/nothing to sync|already up to date/i.test(pane)) {
    console.log('[ok] promus sync — no changes (already in sync)')
  } else {
