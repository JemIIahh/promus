...
const SESSION_A = `promus-cross-a-${process.pid}`
const SESSION_B = `promus-cross-b-${process.pid}`
...
async function main(): Promise<void> {
  const sA = tmuxSession(SESSION_A, 'bun packages/cli/bin/promus')
  let exitCode = 0
  try {
    ...
    // Local cache at ~/.promus/agents/<id>/memory/ already holds the planted
    ...
    const sB = tmuxSession(SESSION_B, 'bun packages/cli/bin/promus')
    try {
