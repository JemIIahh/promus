...
/// @notice Deploy PromusInbox via CREATE2. Same address on testnet + mainnet
...
contract DeployInbox is Script {
    ...
    function run() external returns (PromusInbox inbox) {
        vm.startBroadcast();
        inbox = new PromusInbox{salt: SALT}();
        vm.stopBroadcast();
        console2.log("PromusInbox deployed at:", address(inbox));
