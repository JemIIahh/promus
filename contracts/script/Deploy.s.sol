...
/// @notice Deploy PromusAgentNFT via CREATE2 so mainnet + testnet (or any future
...
contract Deploy is Script {
    ...
    function run(string memory name_, string memory symbol_, address oracle_)
        external
        returns (PromusAgentNFT nft)
    {
        vm.startBroadcast();
        nft = new PromusAgentNFT{salt: SALT}(name_, symbol_, oracle_);
        vm.stopBroadcast();
        console2.log("PromusAgentNFT deployed at:", address(nft));
