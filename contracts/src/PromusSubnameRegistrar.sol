...
/// @notice Permissionless registrar for `<label>.promus.0g` subnames under the
/// `promus.0g` parent on 0G mainnet. Any EOA can call `claim` once per unique
...
/// `setSubnodeRecord` on behalf of promus.0g's registry ownership.
///
...
/// time via `registry.owner(PROMUS_NODE) == promusOwner_`), this is out of scope.
contract PromusSubnameRegistrar {
    /// @dev SANN namehash for `promus.0g`, precomputed:
    ...
    address public immutable promusOwner;
    ...
    /// @param promusOwner_ Registry owner of promus.0g (must pre-approve this contract).
    /// @dev Asserts at construction that the supplied `promusOwner_` matches
    /// `registry.owner(PROMUS_NODE)`. Catches namehash typos AND wrong-chain
    /// deploys in a single revert.
    constructor(address registry_, address resolver_, address promusOwner_) {
        if (ISidRegistry(registry_).owner(PROMUS_NODE) != promusOwner_) revert PromusOwnerMismatch();
        registry = ISidRegistry(registry_);
        defaultResolver = resolver_;
        promusOwner = promusOwner_;
    }

    /// @notice Register `<label>.promus.0g` to `owner`. Reverts if label is
    ...
    /// promus.0g registry owner to write subnames? Re-reads the parent owner
    /// dynamically so an promus.0g transfer correctly flips this false until
